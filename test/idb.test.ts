...
describe('useIDB', () => {
  ...
  it('should set initial value to store', async () => {
    expect(foo()).toBe('initial value')
    expect(await get('foo')).toBe('initial value')
  })

  it('should read initial value from store', async () => {
    await set('bar', 1)
  ...
  it('should update the value using $set', async () => {
    foo.$('new value')
    expect(await get('foo')).toBe('new value')
    expect(foo()).toBe('new value')
  })

  it('should delete the value using $del', async () => {
    await foo.$del()
    expect(foo()).toBeNull()
  })
})
